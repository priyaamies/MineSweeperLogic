...
namespace MineSweeperUnitTests
{
    ...
    public class MineSweeperTests
    {
        ...
        public void CreateBoardTest()
        {
            ...
            Assert.IsTrue(actualCellRows.First().Cells[1].AllCells.Count() == 9, "Number of Cells matched");
        ...
        public void ChangeFlagTypeTest()
        {
            ...
            Assert.IsTrue(flagType == FlagType.Mine, "Flag Type is changed when None is Flagged");

            flagType = PassDifferentFlagType(cell, FlagType.Mine);
            Assert.IsTrue(flagType == FlagType.Flag, "Flag Type is changed when a Mine is flagged?");

            flagType = PassDifferentFlagType(cell, FlagType.Flag);
            Assert.IsTrue(flagType == FlagType.None, "Flag Type is changed when Flagged?");
        ...
        private static FlagType PassDifferentFlagType(Cell cell, FlagType flagType)
        {
            cell.FlagType = flagType;
            var flagTypeValue = Cell.ChangeFlagType(cell);
